...
module "key-vault" {
  ...
  resource_group_name        = data.terraform_remote_state.rg.outputs.resource_group_kube_name
  key_vault_name             = "${random_pet.prefix.id}-keyvault"
  ...
  access_policies = [
    ...
    {
      azure_ad_group_names    = [var.admin_group_obj_id]
      key_permissions         = ["get", "list"]
      secret_permissions      = ["get", "list"]
      certificate_permissions = ["get", "import", "list"]
      storage_permissions     = ["backup", "get", "list", "recover"]
    ...
    {
      azure_ad_service_principal_names = [azurerm_user_assigned_identity.uai.principal_id]
      key_permissions                  = ["get", "list"]
      secret_permissions               = ["get", "list"]
      certificate_permissions          = ["get", "import", "list"]
      storage_permissions              = ["backup", "get", "list", "recover"]
  ...
  existing_vnet_id        = module.hub_network.vnet_id
  existing_subnet_id      = module.hub_network.subnet_ids["jumpbox-subnet"]
}
